...
(function (global) {
  ...
  // Типовий порядок розділів — лише довідка; анкета зберігає порядок автора.
  ...
  function parseLines(lines, opts) {
    ...
    const sections = [];
    const empty = [];
    // Порядок — як у автора анкети (порядок появи графи на сторінці).
    // Раніше тут стояло сортування за ORDER, і розділи перемішувалися.
